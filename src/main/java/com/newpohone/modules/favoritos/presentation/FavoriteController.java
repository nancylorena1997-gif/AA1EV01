package com.newpohone.modules.favoritos.presentation;
...
import com.newpohone.modules.favoritos.domain.FavoriteRedirects;
...
public class FavoriteController {
    ...
    private String guestOrDenied(HttpSession session, String next) {
        if (SessionUsers.current(session) == null) {
            return "redirect:/login?next=" + FavoriteRedirects.safeNext(next);
        }
        return redirectBack(next);
    }

    private String redirectBack(String next) {
        return "redirect:" + FavoriteRedirects.safeNext(next);
    }
}
